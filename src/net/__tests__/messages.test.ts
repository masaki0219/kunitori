...
test('SnapshotMessageSchema preserves full player fields, not just id', () => {
  const payload = {
    ...
    state: {
      ...
      currentPlayer: 0,
      players: [
        { id: 0, name: 'Host', isAI: false, vassals: [], resources: { timber: 0, stone: 0, rice: 0, horse: 0, iron: 0 } },
        { id: 1, name: 'Guest', isAI: false, vassals: [], resources: { timber: 0, stone: 0, rice: 0, horse: 0, iron: 0 } },
      ],
    },
  };
  ...
  expect((parsed.data.state as any).players[0]).toMatchObject({ id: 0, name: 'Host', vassals: [] });
